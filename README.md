...
real TCP, and it's exactly what HW2's chaos server is built to expose.
For HW2 specifically, you also can't shortcut this by reading the
server's source or its exact chaos parameters — you don't have them —
so the only way through is a client that's actually robust, not one
tuned to a known corruption rate or fragmentation pattern. If you can't
explain, live, why your own framing code works, that's a problem you
want to find before the deadline, not during grading.
 
## 7. Questions
 
Office hours / Piazza / email — whatever your instructor has set up for
this course. Don't debug chaos-mode failures alone for hours before
asking; a five-minute conversation usually saves an evening.